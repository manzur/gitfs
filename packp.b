...
init(nil: ref Draw->Context, args: list of string)
{
	...
	if(len args < 2){
		sys->fprint(sys->fildes(2), "usage: programname sha1");
		exit;
	}
	args = tl args;
	entries = Table[list of ref Indexentry].new(17, nil);
...
readsha1(sha1: string): array of byte 
{
...
readbyoffset(packpath: string, off: big): array of byte
{
	...
	if(t == 6){
		...
		basebuf := readbyoffset(packpath, big offset);
		...
		delta := unpack(packfd, size);
		basebuf := readsha1(sha2string(sh));
		unpacked = patch(basebuf, delta);	
	}
	else{
